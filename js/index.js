 let numButtons = document.getElementsByClassName("chooseNum");
 let playersNum;
 for(let i = 0; i < numButtons.length; i++) {
     numButtons[i].addEventListener('click',()=> {
         playersNum = +numButtons[i].id;
         document.querySelector(".chooseNumberOfPlayers").classList.add("hidden");
         document.querySelector(".outer-box").classList.remove("hidden");
     })
 }
...
/* const deck = div({ class: 'deck' });*/
 const deck = document.querySelector(".human_player")
 cardsData.forEach((i) => {
